...
bash -n build.sh serve.sh scripts/check-local.sh scripts/config.sh example.env
shellcheck -x build.sh serve.sh scripts/check-local.sh scripts/config.sh
for check_test in test_benchmark_inputs.py test_mixed_probe.py \
  test_verify_observations.py test_profile_trace_analysis.py \
  test_patch_direction.py test_operator_config.py; do
