...
baseline remained available; this wasn't a GPU fit failure.

Cap 1792 passed startup and 19 API checks. Tool repeats: 7.199/6.808/5.111 seconds; long
...
Warmup took 14.272 seconds and is retained. One launch can't rule out pressure.

Cap 512 passed startup, 19 API checks, one warmup and three 262K repeats. Tool/long
...
The fixture and hash are included so others can repeat the test. Interactivity
mode changes nothing at these explicit settings, so another GPU run wasn't
needed. The scheduler and DSpark algorithms stayed unchanged.
