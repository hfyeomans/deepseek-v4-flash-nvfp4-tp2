...
Tag `recipe-1m-k5` is a clean snapshot of tested serving files and findings.
...
on the test host, independently of Git tags. It was not uploaded to a registry.
See [recovery instructions](running.md#preserved-baseline-and-rollback).

## Working copies after the history change

Clone the clean recipe; merging an old combined branch would restore research
history. Keep research private. For new metric updates, record image/settings
and distinguish them from historical results.
